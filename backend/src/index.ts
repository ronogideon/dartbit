...
app.get('/', (_req, res) => res.json({ service: 'Dartbit API', version: '1.11.77', status: 'running' }));
app.get('/health', (_req, res) => res.json({ status: 'ok', version: '1.11.77', timestamp: new Date().toISOString() }));
...
const server = app.listen(PORT, () => {
  console.log(`\n🚀 Dartbit v1.11.77 running on port ${PORT}\n`);
...
async function patchDatabase() {
  ...
  try {
    ...
    // v1.11.77: reason for a non-package ("other service") prompt payment, carried to the Payment row.
    await safeExec(prisma, 'MpesaTx notes', `ALTER TABLE "MpesaTransaction" ADD COLUMN IF NOT EXISTS "notes" TEXT`);
    // v1.11.77 Fair Use Policy — per-package limits + per-subscriber usage counters.
