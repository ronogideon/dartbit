...
app.get('/', (_req, res) => res.json({ service: 'Dartbit API', version: '1.9.4', status: 'running' }));
app.get('/health', (_req, res) => res.json({ status: 'ok', version: '1.9.4', timestamp: new Date().toISOString() }));
...
const server = app.listen(PORT, () => {
  console.log(`\n🚀 Dartbit v1.9.4 running on port ${PORT}\n`);
  patchDatabase();
  startSessionCleanup();
...
async function patchDatabase() {
  ...
  try {
    ...
    // v1.9.4 — editable templates + system alerts.
