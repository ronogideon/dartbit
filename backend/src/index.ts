...
app.get('/', (_req, res) => res.json({ service: 'Dartbit API', version: '1.10.6', status: 'running' }));
app.get('/health', (_req, res) => res.json({ status: 'ok', version: '1.10.6', timestamp: new Date().toISOString() }));
...
const server = app.listen(PORT, () => {
  console.log(`\n🚀 Dartbit v1.10.6 running on port ${PORT}\n`);
  patchDatabase();
  startSessionCleanup();
...
async function patchDatabase() {
  ...
  try {
    ...
    // v1.10.6 — editable templates + system alerts.
