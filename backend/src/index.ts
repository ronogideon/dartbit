...
app.get('/', (_req, res) => res.json({ service: 'Dartbit API', version: '1.10.60', status: 'running' }));
app.get('/health', (_req, res) => res.json({ status: 'ok', version: '1.10.60', timestamp: new Date().toISOString() }));
...
const server = app.listen(PORT, () => {
  console.log(`\n🚀 Dartbit v1.10.60 running on port ${PORT}\n`);
  patchDatabase();
  startSessionCleanup();
