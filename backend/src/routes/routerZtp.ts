...
async function generateZtpScript(apiKey: string, opts?: { skipCmdScript?: boolean }): Promise<string> {
    ...
    // older ZTP version — any of which can stop captive interception. Tear the whole hotspot down here
    // so §6 recreates it from scratch, exactly like a first provision. Order matters: remove the SERVER
    ...
    if (!loadBalance) {
      ...
      // PROVISIONING-SAFE WAN SETUP. During the import the router must keep a SINGLE, clean default
      // route out the PRIMARY WAN (w1) so its own fetches — login.html, provision-done, the interface
      // report — all succeed. Do NOT put a default route on w2 yet, and do NOT run dartbit-lb inline:
      // both would move/split the router's egress onto w2 mid-script (which may have no internet during
      // setup), timing out every subsequent fetch and aborting provisioning. w1 gets a normal default;
      ...
      // CRITICAL - must precede the LB mark rules below. RouterOS order is: mangle prerouting ->
      // dstnat -> routing decision. An UNAUTHENTICATED hotspot client's first request targets an
      // external site, so at mangle time dst-address-type=!local still matches and the packet gets
      // new-routing-mark=to_wanX. Only AFTERWARDS does dstnat rewrite the destination to the router
      // itself (the hotspot login redirect). The routing decision then honours the mark already set
      // and sends that packet OUT THE WAN instead of delivering it locally - so the login page is
      // never served: no popup, and no internet either. The dst-address-type=!local guard cannot
      // catch this, because it is evaluated BEFORE dstnat rewrites the destination.
      // Verified on T6: re-adding the to_wanX routes reproduced the dead popup on demand; adding
      // this accept rule restored the popup with those routes still present.
      // Scoped to from-client,!auth so PPPoE and AUTHENTICATED hotspot traffic are still marked and
      // load-balanced normally.
      add(`:do { /ip firewall mangle add chain=prerouting action=accept hotspot=\"from-client,!auth\" comment=\"Dartbit LB skip hotspot unauth\" } on-error={}`);
      ...
      // First run: it promotes both DHCP clients to the distance-250 safety nets, so the plain w1 default
      // above is superseded by the managed routing — but only AFTER provisioning has finished fetching.
    ...
    // Dual-WAN LB is activated HERE, as the LAST step — only after every fetch and report above has
    // completed over the clean single-WAN (w1) default route. Running it now promotes both uplinks to
    // the managed distance-250 safety nets and installs balanced/failover routing without a 30s gap.
    // If this router isn't dual-WAN, this is a no-op (no dartbit-lb script exists).
    if (loadBalance) {
      add(`:do { /system script run dartbit-lb } on-error={ :log warning "Dartbit LB: initial activation deferred to scheduler" }`);
    }
    add('');
