...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const elements = await prisma.$queryRawUnsafe(
      `SELECT id, type, name, lat, lng, meta, "parentId", "createdAt" FROM "NetworkElement" WHERE "tenantId"=$1 ORDER BY "createdAt" ASC`, tenantId);
    ...
    sendSuccess(res, { elements, cables, maintenance });
  } catch (err) {
...
router.post('/elements', async (req: AuthRequest, res: Response) => {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO "NetworkElement" (id, "tenantId", type, name, lat, lng, meta, "parentId", "createdBy", "createdAt") VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())`,
      id, tenantId, type, name, lat, lng, meta, parentId, req.user?.userId || null);
    sendSuccess(res, { id });
  } catch (err) {
    sendError(res, err instanceof Error ? err.message : 'Failed', 500);
  }
...
router.patch('/elements/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const meta = req.body?.meta && typeof req.body.meta === 'object' ? JSON.stringify(req.body.meta).slice(0, 4000) : null;
    await prisma.$executeRawUnsafe(
      `UPDATE "NetworkElement" SET name=COALESCE($1,name), lat=COALESCE($2,lat), lng=COALESCE($3,lng), meta=COALESCE($4,meta) WHERE id=$5`,
      name, lat, lng, meta, req.params.id);
